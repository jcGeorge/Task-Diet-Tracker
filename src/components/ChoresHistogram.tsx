...
import type { ChoreEntry, MetaItem } from "../types";

interface ChoresHistogramProps {
  entries: ChoreEntry[];
  chores: MetaItem[];
}
...
export function ChoresHistogram({ entries, chores }: ChoresHistogramProps) {
  ...
  const points = useMemo<HistogramPoint[]>(() => {
    const sortedChores = [...chores].sort((left, right) => left.name.localeCompare(right.name));
    if (sortedChores.length === 0) {
      return [];
    }

    const countById = new Map<string, number>(sortedChores.map((item) => [item.id, 0]));

    for (const entry of entries) {
      const uniqueChoresForEntry = new Set(entry.choreIds);
      for (const choreId of uniqueChoresForEntry) {
        if (!countById.has(choreId)) {
          continue;
        }
        countById.set(choreId, (countById.get(choreId) ?? 0) + 1);
      }
    }

    return sortedChores.map((item) => ({
      id: item.id,
      name: item.name,
  ...
  if (chores.length === 0) {
    return (
      <div className="mb-0">
        <p className="text-secondary mb-2">Add chore options in Metadata to render this chart.</p>
  ...
  return (
    <div className="substances-histogram-wrap">
      <svg
        ...
        role="img"
        aria-label="Chores usage histogram"
      >
        {yTicks.map((tick) => {
