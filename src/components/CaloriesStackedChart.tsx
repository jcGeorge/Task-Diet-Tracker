...
interface CaloriesStackedEntry {
  id: string;
  date: string;
  calories: number;
  notes: string;
...
interface CaloriesStackedChartProps {
  entries: CaloriesStackedEntry[];
...
function buildDailyGroups(entries: CaloriesStackedEntry[]): DailyGroup[] {
  ...
  for (let index = entries.length - 1; index >= 0; index -= 1) {
    ...
    if (!existing) {
      groups.set(isoDate, {
        ...
        segments: [{ id: entry.id, calories: entry.calories, notes: entry.notes }]
      });
      continue;
    }

    existing.totalCalories += entry.calories;
    existing.segments.push({ id: entry.id, calories: entry.calories, notes: entry.notes });
  }

  return [...groups.values()].sort((left, right) => left.isoDate.localeCompare(right.isoDate));
}

export function CaloriesStackedChart({ entries, threshold }: CaloriesStackedChartProps) {
  ...
  if (threshold === null) {
    return (
      <div className="mb-0">
        <p className="text-secondary mb-2">Calorie Limit Per Day is required to render this chart.</p>
  ...
  const maxValue = Math.max(threshold, ...dailyGroups.map((group) => group.totalCalories), 1);
  ...
  return (
    <div className="threshold-chart-wrap">
      <h2 className="h5 mb-3">Calories Chart</h2>
      <svg
        className="threshold-chart-svg"
        viewBox={`0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`}
        preserveAspectRatio="xMidYMid meet"
        role="img"
        aria-label="Calories stacked bar chart"
      >
        {yTicks.map((tick) => {
        ...
        {dailyGroups.map((group, index) => {
          ...
          const segmentPalette = group.totalCalories > threshold ? ABOVE_LIMIT_COLORS : AT_OR_BELOW_LIMIT_COLORS;

          return (
            <g key={group.isoDate}>
              {group.segments.map((segment, segmentIndex) => {
                const startCalories = runningCalories;
                runningCalories += segment.calories;
                const yTop = PADDING.top + plotHeight - (runningCalories / yMax) * plotHeight;
                const yBottom = PADDING.top + plotHeight - (startCalories / yMax) * plotHeight;
